...
function signup(req,res,next){ 
  ...
  User.findOne({$or: [ { email: creds.email }, { username: creds.username } ]}, function(err, user){
    if(err){
      ...
      const signee = new User(creds)
      signee.save(function(err) {
        if(!err){
          req.login({ username: creds.username, password: creds.password }, function(err){
            const isAuthed = req.isAuthenticated()
            if (err) { 
              next(err) 
            }else{
...
function auth(req, res){ 
  const isAuthed = req.isAuthenticated() 
  if(!isAuthed) return res.status(401).send({ isAuthenticated: isAuthed })
