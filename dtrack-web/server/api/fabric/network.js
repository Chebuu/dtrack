...
module.exports = class Network {
  constructor(config){
    ...
    this.wallet = new FileSystemWallet(config.walletPath);
    this.ccp = requireJSON(config.connectionFile);
  }

  async connect(user, channel, contract){
    this.channel = channel;
    this.discovery = this.config.gatewayDiscovery
    this.user = await this.wallet.exists(user) && user;
    ...
    this.network = await this.gateway.getNetwork(channel);
    this.contract = await this.network.getContract(contract);
    return this;
  }

  async invoke(method, args = false, query = false){
    return query ? this.invokeEvaluate(method, args) : this.invokeSubmit(method, args);
  }

  async invokeEvaluate(method, args = null) {
    const response = await this.contract.evaluateTransaction(method, args);
    return response;
  }

  async invokeSubmit(method, args = null) {
    const response = await this.contract.submitTransaction(method, args);
    return response;
  }
};
